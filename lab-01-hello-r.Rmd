...
```{r github-auth-1, echo=FALSE, fig.height=5, fig.width=7, message=TRUE, warning=TRUE, paged.print=TRUE}
...
Answer: 1846 rows and 3 variables. The variables are dataset, x (the x-values) and y (the y-values).
...
```
```{r fig.fullwidth = TRUE}
ggplot(datasaurus_dozen, aes(x =x, y=y, color = dataset))+geom_point() + facet_wrap(~ dataset, ncol = 3) + theme(legend.posiiton = "none")
```
ggplo
